				...
				if apiFunctions.c?
					apiFunctions.c req, res, model, obj, (err, result) ->
						if err
							...
							final h.wrap(result.data, name), (data) ->
				...
				if apiFunctions.ra?
					apiFunctions.ra req, res, model, null, (err, result) ->
						if err
							...
							final h.wrap(result.data, cname), (data) ->
								res.send result.code, data
				else
						...
						if err
							...
							beforeFunctions.ra result, req.user, (err, result) ->
								if err
									...
									if afterFunctions.ra? then return afterFunctions.ra result, req.user
									final h.wrap(result, cname), (data) ->
										res.send 200, data
						else
						...
						if err
							...
							beforeFunctions.ro result, req.user, (err, result) ->
								if err
									...
									if afterFunctions.ro? then return afterFunctions.ro result, req.user
