...
var auth, authenticate, crypto, deserialize, encrypt, h, login, me, mongoose, serialize, utils;
...
authenticate = function(user, cb) {
  ...
  return model.find(toAuth, function(err, result) {
    if (err) {
      return cb(err);
    } else if (result == null) {
...
login = function(user, req, res) {
  return authenticate(user, function(err, user) {
    if (err) {
      ...
      return serialize(user, function(err, toStore) {
        if (err) {
          return res.send(500, err.message);
        } else {
...
auth = module.exports = {
  bind: function(app) {
    ...
    return app.post(url + '/login', function(req, res) {
      var user;
      user = h.unwrap(req.body);
      if (user[pf] == null) {
        return res.send(500, 'password must be present');
